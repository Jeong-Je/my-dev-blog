...
![](/images/9.25/9.25.image2.png)
<figcaption align="center">https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS </figcaption>
...
- `GET`, `HEAD`, `POST` 요청만 가능합니다. <small>(여기에 속하지 않는 `DELETE`요청은 당연히 클라이언트가 요청 전에 `Prefilght`를 통해 서버가 `DELETE`을 허용 하는지 확인해야합니다.)</small>
